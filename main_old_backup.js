...
function createWindow() {
  ...
  // Load shell with navigation, which loads portal inside webview
...
app.on('web-contents-created', (_e, contents) => {
  contents.on('will-attach-webview', (event, params) => {
    ...
    // Don't force preload on webview - let it load normally
...
ipcMain.handle('apps:open', (_e, id) => {
  ...
  if (!win) return false;
  if (meta.kind === 'html') { 
    win.loadFile(path.join(appsRoot, id, meta.entry)); 
    return true; 
  }
  if (meta.kind === 'url') { 
    win.loadURL(meta.entry); 
    return true; 
  }
  return false;
});
